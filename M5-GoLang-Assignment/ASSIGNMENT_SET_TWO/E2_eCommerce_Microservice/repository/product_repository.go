...
func (repo *ProductRepository) AddProduct(product *model.Product) (*model.Product, error) {
...
func (repo *ProductRepository) UpdateStock(id int, stock int) error {
...
func (repo *ProductRepository) GetAllProducts(page, limit int) ([]model.Product, error) {
	rows, err := repo.DB.Query("SELECT id, name, description, price, stock, category_id FROM products LIMIT ? OFFSET ?", limit, (page-1)*limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
